...
export function yamlNameToToolName(yamlName) {
...
export function rewriteYamlRolesToToolNames(content) {
    return content.replace(/role=["'](specline-[a-z0-9-]+)["']/g, (_match, yamlName) => `toolName="${yamlNameToToolName(yamlName)}"`);
}
export function stripNonCursorPlatformSections(content) {
 ...
 */
export function renderDshSkill(content) {
 ...
 */
export function renderFrontendDevPersona(yamlContent, frontendDesignMarkdown) {

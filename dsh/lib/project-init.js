...
export const INIT_ARGS = ['init', '--platform', 'none'];
...
export function shouldPromptInit(kind) {
    return kind === 'web';
}
export function buildInitInvocation() {
    return { command: INIT_COMMAND, args: [...INIT_ARGS] };
}
export function describeInitPolicy(kind) {
    if (kind === 'web') {
        return { prompt: true, allowInit: true, autoInit: false };
    }
    return { prompt: false, allowInit: false, autoInit: false };
}
export function createDefaultInitRunner(spawnFn = (command, args, options) => spawnSync(command, [...args], options)) {
    return (invocation) => {

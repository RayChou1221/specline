...
export const ROLE_PERSONA_SOURCES: Record<RoleToolName, string> = {
...
const ROLE_TOOL_NAME_SET = new Set<string>(ROLE_TOOL_NAMES);

export function filterChildSessionTools(tools: readonly string[]): string[] {
  return tools.filter((name) => !ROLE_TOOL_NAME_SET.has(name));
}

export function createRoleToolConfig(toolName: RoleToolName): RoleToolConfig {
...
export function createRoleToolConfigs(): RoleToolConfig[] {

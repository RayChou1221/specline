...
| 表达目标 | 路由 |
| --- | --- |
| 简单关系、临时草图、当前对话即可看懂 | Explore 内直接使用 ASCII |
...
当在 Agent 模式下运行时，可以通过 调用 specline_* 角色工具，toolName="specline_explore_assistant" 将压力测试分派给子 Agent——它不带上下文偏见，以全新视角审视设计，效果更好。

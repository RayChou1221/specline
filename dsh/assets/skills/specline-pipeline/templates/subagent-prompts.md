...
**共享测试路径模式（引用 Gate，勿另写正则）**：`Testable: true` 的测试落点由 `core/gates/pipeline-gate.sh` 的共享匹配器（`list_testable_declared_tests`）判定。Prompt 只引用同一列表，不要另写一套会漂移的完整正则引擎。命中：`tests/unit/` 或 `tests/models/` 下任意文件、`*_test.go`、`*.test.ts`/`*.test.tsx`/`*.test.js`/`*.test.jsx`、`*.spec.ts`/`*.spec.tsx`/`*.spec.js`/`*.spec.jsx`、`src/*/tests.rs`。仍禁止写入 `tests/integration/` 与 `tests/e2e/`。
...
「只改 Files」与「必须写测试」是同一条规则：测试文件就是本任务 `Files:` 中命中 Gate 共享测试路径模式的那些路径。模式与 `core/gates/pipeline-gate.sh` 的共享匹配器（`list_testable_declared_tests`）相同，不要另写一套会漂移的完整正则：`tests/unit/` 或 `tests/models/` 下任意文件、`*_test.go`、`*.test.ts`/`*.test.tsx`/`*.test.js`/`*.test.jsx`、`*.spec.ts`/`*.spec.tsx`/`*.spec.js`/`*.spec.jsx`、`src/*/tests.rs`。只把测试写到这些已声明路径；目录不存在则创建。仍禁止写入 `tests/integration/` 与 `tests/e2e/`。

你必须按以下 TDD 循环编写代码：

### RED 阶段
1. 分析 Spec 中本任务覆盖的 Scenario，提取需要测试的逻辑单元
2. 在本任务 `Files:` 已声明且命中上述共享模式的测试路径编写测试；父目录不存在则创建。不要改写到未声明的落点，也不再限制为只能写 `tests/unit/` 或 `tests/models/`
...
10. 运行测试，确保持续 PASS
11. 如果需要，补充缺失的边界条件测试（仍只写入本任务 `Files:` 中已声明的测试路径）

## 关键约束
1. 只修改本任务 Files 范围内的文件；其中命中共享模式的路径必须被创建并写入测试
2. 不修改其他任务负责的文件
3. 与已完成任务的接口约定必须遵守（参考已生成的接口/类型定义文件）
4. 确认过 design.md 中的技术决策后再动手
5. 不得修改 tests/integration/ 或 tests/e2e/ 目录下的文件（属于 test-writer）
6. **完成后必须将 tasks.md 中本任务的 `[ ]` 改为 `[x]`**（方便断点续跑识别进度）
...
完成后在 specline/changes/${changeName}/.tmp/task-${task.id}-result.json 写入：
{
  ...
  "status": "completed",
  "files_changed": [...],
  "test_files": ["<本任务 Files 中命中共享模式的测试路径>", ...],
  "tests_passed": true,
  "summary": "..."
}
```
...
3. 与已完成任务的接口约定必须遵守（参考已生成的接口/类型定义文件）
4. 确认过 design.md 中的技术决策后再动手
5. 若本任务 `Files:` 含命中 Gate 共享测试路径模式的路径（与 `pipeline-gate.sh` 的 `list_testable_declared_tests` 相同：`tests/unit/`/`tests/models/`、`*_test.go`、`*.test.ts`/`*.test.tsx`/`*.test.js`/`*.test.jsx`、`*.spec.ts`/`*.spec.tsx`/`*.spec.js`/`*.spec.jsx`、`src/*/tests.rs`），则必须写入那些测试文件；目录不存在则创建。不要另写一套正则去推断其它落点。不得写入 `tests/integration/` 或 `tests/e2e/`。
6. **完成后必须将 tasks.md 中本任务的 `[ ]` 改为 `[x]`**（方便断点续跑识别进度）

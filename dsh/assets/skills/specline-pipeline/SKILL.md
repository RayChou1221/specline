...
**读取方式**：在每个 HG 触发前读取 `specline/config.yaml` 的 `pipeline.human_gate_policy` 字段（通过 grep/yq），若策略指示跳过当前 HG，则直接写入 `.pipeline-state.json` 中对应 `human_gate_N.passed = true`，不调用 直接向用户提问。

**子 Agent 确认行为**：当策略为 `minimal` 或 `none` 时，编排者应在派发子 Agent 时传递 `HUMAN_GATE_POLICY=<policy>` 上下文，告知子 Agent 跳过所有 直接向用户提问 交互（如 spec_ambiguity 暂停、skill 级确认等），直接采用默认安全选项继续执行。编排者自身在测试失败处理中的 spec_ambiguity 暂停也应改为记录 WARNING 事件日志后继续。
...
- `none`：不打断用户；只记录假设、风险和 deferred questions，不要求用户确认，也不引入任何 直接向用户提问。
...
调用 specline_* 角色工具，toolName="specline_spec_creator"，描述中传入 change name、自然语言需求、`HUMAN_GATE_POLICY=<policy>`，以及 Ambiguity Scan 产生的可选 `clarification_context`。让 specline-spec-creator 根据内联模板直接生成，并把 confirmed decisions、assumptions、deferred questions、risk level 和 warnings 写入规划 Artifact。
...
Spec Gate 通过后，`full` 策略使用 直接向用户提问 请求确认。展示内容包括：需求提案摘要、功能需求列表、任务拆解概览（含并行组），以及 Ambiguity Scan/Spec Creator 产出的：

- 用户已确认的关键决策。
- 未经确认但将采用的 assumptions 和 recommended defaults。
- deferred questions、unresolved items、warning defaults 及其实现风险。

Human Gate 1 具体交互：使用 直接向用户提问，title="确认 Spec 和任务规划"，选项：`approve`（确认通过，继续编码）/ `reject`（不通过，手动修改后重新审核）。若 HG1 因 `minimal` 或 `none` 跳过，编排者不得补问用户；只记录 warnings/assumptions，并保证后续 reviewer/gate 能看到这些风险。
...
调用 specline_* 角色工具，toolName="specline_test_writer"，prompt 中包含 changeName/Spec/Design/Tasks 路径和黑盒约束。test-writer 从 `design.md` 的「对外接口契约」章节获取 CLI 命令/HTTP 端点/模块导出签名（若章节不存在则在 prompt 中注明无需契约），从 spec.md 获取行为验收标准（WHEN/THEN）。specline-test-writer 只编写 tests/integration/** 和 tests/e2e/** 下的测试，产出 test-code-result.json。
...
**派发规则**：
- 同角色组内有 **≥2 个任务** → 使用 Template 4（批量执行），一个 调用 specline_* 角色工具 处理该组全部任务
...
仅当 code-review.json 中 warnings > 0 且 errors = 0 时，使用 直接向用户提问 询问是否人工复核（`skip` 自动继续 / `review` 展示警告详情）。
...
全部测试通过后，使用 直接向用户提问 请求归档确认（`archive` 执行归档 / `cancel` 暂停流水线）。
...
5. **特殊规则**：
   ...
   - Hook 阻断 → 先诊断原因 → 与用户沟通（直接向用户提问）→ 修复后重试 → 绝不静默降级
...
2. **与用户沟通**：使用 直接向用户提问 展示诊断结果和修复选项（auto_fix / manual_fix / skip_agent）
...
如果只有一个未完成流水线，使用 直接向用户提问 做一次快速确认：title="恢复流水线"，展示变更名称、已完成阶段、未完成阶段，选项：`continue`（继续执行）/ `cancel`（取消）。

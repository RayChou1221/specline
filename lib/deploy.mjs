...
export function getSharedSpeclineManifest(packageRoot = PACKAGE_ROOT) {
  ...
  for (const { rel, abs } of walkDir(
    join(packageRoot, 'core', 'runtimes', 'drawio'),
    'specline/runtime/drawio',
  )) {
    manifest.set(rel, { source: abs });
  }

  for (const file of ['diagram.mjs', 'paths.mjs']) {
    manifest.set(`specline/runtime/${file}`, { source: join(packageRoot, 'lib', file) });
  }
  for (const { rel, abs } of walkDir(
    join(packageRoot, 'lib', 'diagram'),
